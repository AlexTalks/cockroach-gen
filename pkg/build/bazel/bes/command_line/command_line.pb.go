...
func (m *Option) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if len(m.MetadataTags) > 0 {
		dAtA4 := make([]byte, len(m.MetadataTags)*10)
		var j3 int
		...
		i = encodeVarintCommandLine(dAtA, i, uint64(j3))
		i--
		dAtA[i] = 0x2a
	}
	if len(m.EffectTags) > 0 {
		dAtA6 := make([]byte, len(m.EffectTags)*10)
		var j5 int
		...
		i = encodeVarintCommandLine(dAtA, i, uint64(j5))
		i--
		dAtA[i] = 0x22
	}
...
func (m *CommandLineSection) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			...
			m.SectionType = &CommandLineSection_ChunkList{v}
			iNdEx = postIndex
		case 3:
			if wireType != 2 {
			...
			m.SectionType = &CommandLineSection_OptionList{v}
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skipCommandLine(dAtA[iNdEx:])
