...
func (e *InsertExpr) SetChild(nth int, child opt.Expr) {
	...
	case 1:
		...
		return
	case 2:
		e.FastPathUniqueChecks = *child.(*FastPathUniqueChecksExpr)
		return
	case 3:
...
// FastPathUniqueChecksExpr is a list of uniqueness check Selects which could be
// converted into constrained Scans from which KV requests can be generated
// to be used in place of regular UniqueChecks.
type FastPathUniqueChecksExpr []FastPathUniqueChecksItem

var EmptyFastPathUniqueChecksExpr = FastPathUniqueChecksExpr{}

var _ opt.ScalarExpr = &FastPathUniqueChecksExpr{}

func (e *FastPathUniqueChecksExpr) Rank() opt.ScalarRank {
	panic(errors.AssertionFailedf("lists have no rank"))
}

func (e *FastPathUniqueChecksExpr) Op() opt.Operator {
	return opt.FastPathUniqueChecksOp
}

func (e *FastPathUniqueChecksExpr) ChildCount() int {
	return len(*e)
}

func (e *FastPathUniqueChecksExpr) Child(nth int) opt.Expr {
	return &(*e)[nth]
}

func (e *FastPathUniqueChecksExpr) Private() interface{} {
	return nil
}

func (e *FastPathUniqueChecksExpr) String() string {
	f := makeExprFmtCtxForString(ExprFmtHideQualifications, nil, nil)
	f.FormatExpr(e)
	return f.Buffer.String()
}

func (e *FastPathUniqueChecksExpr) SetChild(nth int, child opt.Expr) {
	(*e)[nth] = *child.(*FastPathUniqueChecksItem)
}

func (e *FastPathUniqueChecksExpr) DataType() *types.T {
	return types.Any
}
...
type FastPathUniqueChecksItem struct {
	Check RelExpr
...
var _ opt.ScalarExpr = &FastPathUniqueChecksItem{}

func (e *FastPathUniqueChecksItem) Rank() opt.ScalarRank {
	panic(errors.AssertionFailedf("list items have no rank"))
}

func (e *FastPathUniqueChecksItem) Op() opt.Operator {
	return opt.FastPathUniqueChecksItemOp
}

func (e *FastPathUniqueChecksItem) ChildCount() int {
	return 1
}

func (e *FastPathUniqueChecksItem) Child(nth int) opt.Expr {
	switch nth {
	case 0:
		return e.Check
	}
...
func (e *FastPathUniqueChecksItem) Private() interface{} {
	return &e.FastPathUniqueChecksItemPrivate
}

func (e *FastPathUniqueChecksItem) String() string {
	f := makeExprFmtCtxForString(ExprFmtHideQualifications, nil, nil)
	f.FormatExpr(e)
	return f.Buffer.String()
}

func (e *FastPathUniqueChecksItem) SetChild(nth int, child opt.Expr) {
	switch nth {
	case 0:
		e.Check = child.(RelExpr)
		return
...
func (e *FastPathUniqueChecksItem) DataType() *types.T {
	return e.Typ
}
...
// execbuilder phase.
type FastPathUniqueChecksItemPrivate struct {
	ReferencedTableID      opt.TableID
...
// LockExpr evaluates a relational input expression, and locks rows in the given
...
var _ RelExpr = &LockExpr{}

func (e *LockExpr) Op() opt.Operator {
	return opt.LockOp
}

func (e *LockExpr) ChildCount() int {
	return 1
}

func (e *LockExpr) Child(nth int) opt.Expr {
	switch nth {
	case 0:
		return e.Input
	}
...
func (e *LockExpr) Private() interface{} {
	return &e.LockPrivate
}

func (e *LockExpr) String() string {
	f := makeExprFmtCtxForString(ExprFmtHideQualifications, e.Memo(), nil)
	f.FormatExpr(e)
	return f.Buffer.String()
}

func (e *LockExpr) SetChild(nth int, child opt.Expr) {
	switch nth {
	case 0:
		e.Input = child.(RelExpr)
		return
...
func (e *LockExpr) Memo() *Memo {
	return e.grp.memo()
}

func (e *LockExpr) Relational() *props.Relational {
	return e.grp.relational()
}

func (e *LockExpr) FirstExpr() RelExpr {
	return e.grp.firstExpr()
}

func (e *LockExpr) NextExpr() RelExpr {
	return e.next
}

func (e *LockExpr) RequiredPhysical() *physical.Required {
	return e.grp.bestProps().required
}

func (e *LockExpr) ProvidedPhysical() *physical.Provided {
	return e.grp.bestProps().provided
}

func (e *LockExpr) Cost() Cost {
	return e.grp.bestProps().cost
}

func (e *LockExpr) group() exprGroup {
	return e.grp
}

func (e *LockExpr) bestProps() *bestProps {
	return e.grp.bestProps()
}

func (e *LockExpr) setNext(member RelExpr) {
	if e.next != nil {
		panic(errors.AssertionFailedf("expression already has its next defined: %s", e))
	}
	e.next = member
...
func (e *LockExpr) setGroup(member RelExpr) {
	if e.grp != nil {
		panic(errors.AssertionFailedf("expression is already in a group: %s", e))
	}
	e.grp = member.group()
...
var _ exprGroup = &lockGroup{}

func (g *lockGroup) memo() *Memo {
	return g.mem
}

func (g *lockGroup) relational() *props.Relational {
	return &g.rel
}

func (g *lockGroup) firstExpr() RelExpr {
	return &g.first
}

func (g *lockGroup) bestProps() *bestProps {
	return &g.best
}
...
type LookupJoinPrivate struct {
	...
	// columns are not reducing the selectivity of the join, but are just added
	// to facilitate index lookups, they should not be used in determining join
	// selectivity. DerivedEquivCols could also include predicates derived from
	// foreign key constraints which are used to facilitate index lookups. The
	// additional terms should not affect selectivity so that we have consistent
	// join relation stats for fairness in costing lookup join vs. joins which do
	// not use these predicates.
...
// BarrierExpr is a relational operator that simply passes through its input columns
// with no changes. It serves as an optimization barrier, and is not included in
// the final execution plan.
type BarrierExpr struct {
	Input RelExpr
...
var _ RelExpr = &BarrierExpr{}

func (e *BarrierExpr) Op() opt.Operator {
	return opt.BarrierOp
}

func (e *BarrierExpr) ChildCount() int {
	return 1
}

func (e *BarrierExpr) Child(nth int) opt.Expr {
	switch nth {
	case 0:
		return e.Input
	}
...
func (e *BarrierExpr) Private() interface{} {
	return nil
}

func (e *BarrierExpr) String() string {
	f := makeExprFmtCtxForString(ExprFmtHideQualifications, e.Memo(), nil)
	f.FormatExpr(e)
	return f.Buffer.String()
}

func (e *BarrierExpr) SetChild(nth int, child opt.Expr) {
	switch nth {
	case 0:
		e.Input = child.(RelExpr)
		return
...
func (e *BarrierExpr) Memo() *Memo {
	return e.grp.memo()
}

func (e *BarrierExpr) Relational() *props.Relational {
	return e.grp.relational()
}

func (e *BarrierExpr) FirstExpr() RelExpr {
	return e.grp.firstExpr()
}

func (e *BarrierExpr) NextExpr() RelExpr {
	return e.next
}

func (e *BarrierExpr) RequiredPhysical() *physical.Required {
	return e.grp.bestProps().required
}

func (e *BarrierExpr) ProvidedPhysical() *physical.Provided {
	return e.grp.bestProps().provided
}

func (e *BarrierExpr) Cost() Cost {
	return e.grp.bestProps().cost
}

func (e *BarrierExpr) group() exprGroup {
	return e.grp
}

func (e *BarrierExpr) bestProps() *bestProps {
	return e.grp.bestProps()
}

func (e *BarrierExpr) setNext(member RelExpr) {
	if e.next != nil {
		panic(errors.AssertionFailedf("expression already has its next defined: %s", e))
	}
	e.next = member
...
func (e *BarrierExpr) setGroup(member RelExpr) {
	if e.grp != nil {
		panic(errors.AssertionFailedf("expression is already in a group: %s", e))
	}
	e.grp = member.group()
...
var _ exprGroup = &barrierGroup{}

func (g *barrierGroup) memo() *Memo {
	return g.mem
}

func (g *barrierGroup) relational() *props.Relational {
	return &g.rel
}

func (g *barrierGroup) firstExpr() RelExpr {
	return &g.first
}

func (g *barrierGroup) bestProps() *bestProps {
	return &g.best
}
...
// rules can be used across both expressions.
...
type ExistsPrivate struct {
	...
	// optimization.
	LazyEvalProjectionCol opt.ColumnID
	SubqueryPrivate
}
...
type MergeAggregatedStmtMetadataExpr struct {
	Input opt.ScalarExpr
...
var _ opt.ScalarExpr = &MergeAggregatedStmtMetadataExpr{}

func (e *MergeAggregatedStmtMetadataExpr) Rank() opt.ScalarRank {
	return e.rank
}

func (e *MergeAggregatedStmtMetadataExpr) Op() opt.Operator {
	return opt.MergeAggregatedStmtMetadataOp
}

func (e *MergeAggregatedStmtMetadataExpr) ChildCount() int {
	return 1
}

func (e *MergeAggregatedStmtMetadataExpr) Child(nth int) opt.Expr {
	switch nth {
	case 0:
		return e.Input
	}
...
func (e *MergeAggregatedStmtMetadataExpr) Private() interface{} {
	return nil
}

func (e *MergeAggregatedStmtMetadataExpr) String() string {
	f := makeExprFmtCtxForString(ExprFmtHideQualifications, nil, nil)
	f.FormatExpr(e)
	return f.Buffer.String()
}

func (e *MergeAggregatedStmtMetadataExpr) SetChild(nth int, child opt.Expr) {
	switch nth {
	case 0:
		e.Input = child.(opt.ScalarExpr)
		return
...
func (e *MergeAggregatedStmtMetadataExpr) DataType() *types.T {
	return e.Typ
}

type MergeStatsMetadataExpr struct {
	Input opt.ScalarExpr
...
var _ opt.ScalarExpr = &MergeStatsMetadataExpr{}

func (e *MergeStatsMetadataExpr) Rank() opt.ScalarRank {
	return e.rank
}

func (e *MergeStatsMetadataExpr) Op() opt.Operator {
	return opt.MergeStatsMetadataOp
}

func (e *MergeStatsMetadataExpr) ChildCount() int {
	return 1
}

func (e *MergeStatsMetadataExpr) Child(nth int) opt.Expr {
	switch nth {
	case 0:
		return e.Input
	}
...
func (e *MergeStatsMetadataExpr) Private() interface{} {
	return nil
}

func (e *MergeStatsMetadataExpr) String() string {
	f := makeExprFmtCtxForString(ExprFmtHideQualifications, nil, nil)
	f.FormatExpr(e)
	return f.Buffer.String()
}

func (e *MergeStatsMetadataExpr) SetChild(nth int, child opt.Expr) {
	switch nth {
	case 0:
		e.Input = child.(opt.ScalarExpr)
		return
...
func (e *MergeStatsMetadataExpr) DataType() *types.T {
	return e.Typ
}

type MergeStatementStatsExpr struct {
	Input opt.ScalarExpr
...
var _ opt.ScalarExpr = &MergeStatementStatsExpr{}

func (e *MergeStatementStatsExpr) Rank() opt.ScalarRank {
	return e.rank
}

func (e *MergeStatementStatsExpr) Op() opt.Operator {
	return opt.MergeStatementStatsOp
}

func (e *MergeStatementStatsExpr) ChildCount() int {
	return 1
}

func (e *MergeStatementStatsExpr) Child(nth int) opt.Expr {
	switch nth {
	case 0:
		return e.Input
	}
...
func (e *MergeStatementStatsExpr) Private() interface{} {
	return nil
}

func (e *MergeStatementStatsExpr) String() string {
	f := makeExprFmtCtxForString(ExprFmtHideQualifications, nil, nil)
	f.FormatExpr(e)
	return f.Buffer.String()
}

func (e *MergeStatementStatsExpr) SetChild(nth int, child opt.Expr) {
	switch nth {
	case 0:
		e.Input = child.(opt.ScalarExpr)
		return
...
func (e *MergeStatementStatsExpr) DataType() *types.T {
	return e.Typ
}

type MergeTransactionStatsExpr struct {
	Input opt.ScalarExpr
...
var _ opt.ScalarExpr = &MergeTransactionStatsExpr{}

func (e *MergeTransactionStatsExpr) Rank() opt.ScalarRank {
	return e.rank
}

func (e *MergeTransactionStatsExpr) Op() opt.Operator {
	return opt.MergeTransactionStatsOp
}

func (e *MergeTransactionStatsExpr) ChildCount() int {
	return 1
}

func (e *MergeTransactionStatsExpr) Child(nth int) opt.Expr {
	switch nth {
	case 0:
		return e.Input
	}
...
func (e *MergeTransactionStatsExpr) Private() interface{} {
	return nil
}

func (e *MergeTransactionStatsExpr) String() string {
	f := makeExprFmtCtxForString(ExprFmtHideQualifications, nil, nil)
	f.FormatExpr(e)
	return f.Buffer.String()
}

func (e *MergeTransactionStatsExpr) SetChild(nth int, child opt.Expr) {
	switch nth {
	case 0:
		e.Input = child.(opt.ScalarExpr)
		return
...
func (e *MergeTransactionStatsExpr) DataType() *types.T {
	return e.Typ
}
...
// UDFCallExpr invokes a user-defined function. The UDFPrivate field contains a
// pointer to the definition of the UDF.
...
var _ opt.ScalarExpr = &UDFCallExpr{}

func (e *UDFCallExpr) Rank() opt.ScalarRank {
	return e.rank
}

func (e *UDFCallExpr) Op() opt.Operator {
	return opt.UDFCallOp
}

func (e *UDFCallExpr) ChildCount() int {
	return 1
}

func (e *UDFCallExpr) Child(nth int) opt.Expr {
	switch nth {
	case 0:
		return &e.Args
	}
...
func (e *UDFCallExpr) Private() interface{} {
	return &e.UDFCallPrivate
}

func (e *UDFCallExpr) String() string {
	f := makeExprFmtCtxForString(ExprFmtHideQualifications, nil, nil)
	f.FormatExpr(e)
	return f.Buffer.String()
}

func (e *UDFCallExpr) SetChild(nth int, child opt.Expr) {
	switch nth {
	case 0:
		e.Args = *child.(*ScalarListExpr)
		return
...
func (e *UDFCallExpr) DataType() *types.T {
	return e.Typ
}

type UDFCallPrivate struct {
	// Def points to the UDF SQL body.
	Def *UDFDefinition
}

// TxnControlExpr allows PL/pgSQL stored procedures to pause their execution, commit
// or rollback the current transaction, and then continue execution in the new
// transaction.
type TxnControlExpr struct {
	// Args contains the scalar expressions given as arguments to the
	// "continuation" stored procedure invocation.
	Args ScalarListExpr
	TxnControlPrivate
...
var _ opt.ScalarExpr = &TxnControlExpr{}

func (e *TxnControlExpr) Rank() opt.ScalarRank {
	return e.rank
}

func (e *TxnControlExpr) Op() opt.Operator {
	return opt.TxnControlOp
}

func (e *TxnControlExpr) ChildCount() int {
	return 1
}

func (e *TxnControlExpr) Child(nth int) opt.Expr {
	switch nth {
	case 0:
		return &e.Args
	}
...
func (e *TxnControlExpr) Private() interface{} {
	return &e.TxnControlPrivate
}

func (e *TxnControlExpr) String() string {
	f := makeExprFmtCtxForString(ExprFmtHideQualifications, nil, nil)
	f.FormatExpr(e)
	return f.Buffer.String()
}

func (e *TxnControlExpr) SetChild(nth int, child opt.Expr) {
	switch nth {
	case 0:
		e.Args = *child.(*ScalarListExpr)
		return
...
func (e *TxnControlExpr) DataType() *types.T {
	return e.Typ
}

type TxnControlPrivate struct {
	// TxnOp indicates whether the current transaction should be committed or
	// aborted.
	...
	// OutCols is used when building the plan for the continuation SP.
	OutCols opt.ColList

	// Def stores the procedure that will resume execution of the parent SP in
	// the new transaction.
	Def *UDFDefinition
}
...
var _ RelExpr = &CallExpr{}

func (e *CallExpr) Op() opt.Operator {
	return opt.CallOp
}

func (e *CallExpr) ChildCount() int {
	return 1
}

func (e *CallExpr) Child(nth int) opt.Expr {
	switch nth {
	case 0:
		return e.Proc
	}
...
func (e *CallExpr) Private() interface{} {
	return &e.CallPrivate
}
...
func (e *CallExpr) SetChild(nth int, child opt.Expr) {
	switch nth {
	case 0:
		e.Proc = child.(opt.ScalarExpr)
		return
	}
	panic(errors.AssertionFailedf("child index out of range"))
}

func (e *CallExpr) Memo() *Memo {
	return e.grp.memo()
}

func (e *CallExpr) Relational() *props.Relational {
	return e.grp.relational()
}

func (e *CallExpr) FirstExpr() RelExpr {
	return e.grp.firstExpr()
}

func (e *CallExpr) NextExpr() RelExpr {
	return e.next
}

func (e *CallExpr) RequiredPhysical() *physical.Required {
	return e.grp.bestProps().required
}

func (e *CallExpr) ProvidedPhysical() *physical.Provided {
	return e.grp.bestProps().provided
}

func (e *CallExpr) Cost() Cost {
	return e.grp.bestProps().cost
}

func (e *CallExpr) group() exprGroup {
	return e.grp
}

func (e *CallExpr) bestProps() *bestProps {
	return e.grp.bestProps()
}

func (e *CallExpr) setNext(member RelExpr) {
	if e.next != nil {
		panic(errors.AssertionFailedf("expression already has its next defined: %s", e))
	}
	e.next = member
...
func (e *CallExpr) setGroup(member RelExpr) {
	if e.grp != nil {
		panic(errors.AssertionFailedf("expression is already in a group: %s", e))
	}
	e.grp = member.group()
...
var _ exprGroup = &callGroup{}

func (g *callGroup) memo() *Memo {
	return g.mem
}

func (g *callGroup) relational() *props.Relational {
	return &g.rel
}

func (g *callGroup) firstExpr() RelExpr {
	return &g.first
}

func (g *callGroup) bestProps() *bestProps {
	return &g.best
}

type CallPrivate struct {
	// Columns stores the column IDs for the statement result columns.
	Columns opt.ColList
}
...
func (m *Memo) MemoizeInsert(
	...
	grp := &insertGroup{mem: m, first: InsertExpr{
		...
		FastPathUniqueChecks: fastPathUniqueChecks,
		FKChecks:             fKChecks,
		MutationPrivate:      *mutationPrivate,
	}}
	e := &grp.first
...
func (m *Memo) MemoizeLock(
	input RelExpr,
	lockPrivate *LockPrivate,
) RelExpr {
	const size = int64(unsafe.Sizeof(lockGroup{}))
	grp := &lockGroup{mem: m, first: LockExpr{
		Input:       input,
		LockPrivate: *lockPrivate,
	}}
	e := &grp.first
	e.grp = grp
	interned := m.interner.InternLock(e)
	if interned == e {
		if m.newGroupFn != nil {
			m.newGroupFn(e)
		}
		m.logPropsBuilder.buildLockProps(e, &grp.rel)
		grp.rel.Populated = true
		m.memEstimate += size
		m.CheckExpr(e)
	}
...
func (m *Memo) MemoizeBarrier(
	input RelExpr,
) RelExpr {
	const size = int64(unsafe.Sizeof(barrierGroup{}))
	grp := &barrierGroup{mem: m, first: BarrierExpr{
		Input: input,
	}}
	e := &grp.first
	e.grp = grp
	interned := m.interner.InternBarrier(e)
	if interned == e {
		if m.newGroupFn != nil {
			m.newGroupFn(e)
		}
		m.logPropsBuilder.buildBarrierProps(e, &grp.rel)
		grp.rel.Populated = true
		m.memEstimate += size
		m.CheckExpr(e)
	}
...
func (m *Memo) MemoizeMergeAggregatedStmtMetadata(
	input opt.ScalarExpr,
) *MergeAggregatedStmtMetadataExpr {
	const size = int64(unsafe.Sizeof(MergeAggregatedStmtMetadataExpr{}))
	e := &MergeAggregatedStmtMetadataExpr{
		Input: input,
		rank:  m.NextRank(),
	}
	e.Typ = InferType(m, e)
	interned := m.interner.InternMergeAggregatedStmtMetadata(e)
	if interned == e {
		if m.newGroupFn != nil {
			m.newGroupFn(e)
		}
...
func (m *Memo) MemoizeMergeStatsMetadata(
	input opt.ScalarExpr,
) *MergeStatsMetadataExpr {
	const size = int64(unsafe.Sizeof(MergeStatsMetadataExpr{}))
	e := &MergeStatsMetadataExpr{
		Input: input,
		rank:  m.NextRank(),
	}
	e.Typ = InferType(m, e)
	interned := m.interner.InternMergeStatsMetadata(e)
	if interned == e {
		if m.newGroupFn != nil {
			m.newGroupFn(e)
		}
...
func (m *Memo) MemoizeMergeStatementStats(
	input opt.ScalarExpr,
) *MergeStatementStatsExpr {
	const size = int64(unsafe.Sizeof(MergeStatementStatsExpr{}))
	e := &MergeStatementStatsExpr{
		Input: input,
		rank:  m.NextRank(),
	}
	e.Typ = InferType(m, e)
	interned := m.interner.InternMergeStatementStats(e)
	if interned == e {
		if m.newGroupFn != nil {
			m.newGroupFn(e)
		}
...
func (m *Memo) MemoizeMergeTransactionStats(
	input opt.ScalarExpr,
) *MergeTransactionStatsExpr {
	const size = int64(unsafe.Sizeof(MergeTransactionStatsExpr{}))
	e := &MergeTransactionStatsExpr{
		Input: input,
		rank:  m.NextRank(),
	}
	e.Typ = InferType(m, e)
	interned := m.interner.InternMergeTransactionStats(e)
	if interned == e {
		if m.newGroupFn != nil {
			m.newGroupFn(e)
		}
...
func (m *Memo) MemoizeUDFCall(
	args ScalarListExpr,
	uDFCallPrivate *UDFCallPrivate,
) *UDFCallExpr {
	const size = int64(unsafe.Sizeof(UDFCallExpr{}))
	e := &UDFCallExpr{
		Args:           args,
		UDFCallPrivate: *uDFCallPrivate,
		rank:           m.NextRank(),
	...
	e.Typ = InferType(m, e)
	interned := m.interner.InternUDFCall(e)
	if interned == e {
		if m.newGroupFn != nil {
			m.newGroupFn(e)
		}
...
func (m *Memo) MemoizeTxnControl(
	args ScalarListExpr,
	txnControlPrivate *TxnControlPrivate,
) *TxnControlExpr {
	const size = int64(unsafe.Sizeof(TxnControlExpr{}))
	e := &TxnControlExpr{
		Args:              args,
		TxnControlPrivate: *txnControlPrivate,
		rank:              m.NextRank(),
	...
	e.Typ = InferType(m, e)
	interned := m.interner.InternTxnControl(e)
	if interned == e {
		if m.newGroupFn != nil {
			m.newGroupFn(e)
		}
...
func (m *Memo) MemoizeCall(
	proc opt.ScalarExpr,
	callPrivate *CallPrivate,
) RelExpr {
	const size = int64(unsafe.Sizeof(callGroup{}))
	grp := &callGroup{mem: m, first: CallExpr{
		Proc:        proc,
		CallPrivate: *callPrivate,
	}}
	e := &grp.first
	e.grp = grp
	interned := m.interner.InternCall(e)
	if interned == e {
		if m.newGroupFn != nil {
			m.newGroupFn(e)
		}
		m.logPropsBuilder.buildCallProps(e, &grp.rel)
		grp.rel.Populated = true
		m.memEstimate += size
		m.CheckExpr(e)
	}
...
func (m *Memo) AddLockToGroup(e *LockExpr, grp RelExpr) *LockExpr {
	const size = int64(unsafe.Sizeof(LockExpr{}))
	interned := m.interner.InternLock(e)
	if interned == e {
		e.setGroup(grp)
		m.memEstimate += size
...
func (m *Memo) AddBarrierToGroup(e *BarrierExpr, grp RelExpr) *BarrierExpr {
	const size = int64(unsafe.Sizeof(BarrierExpr{}))
	interned := m.interner.InternBarrier(e)
	if interned == e {
		e.setGroup(grp)
		m.memEstimate += size
...
func (m *Memo) AddCallToGroup(e *CallExpr, grp RelExpr) *CallExpr {
	const size = int64(unsafe.Sizeof(CallExpr{}))
	interned := m.interner.InternCall(e)
	if interned == e {
		e.setGroup(grp)
		m.memEstimate += size
...
func (in *interner) InternInsert(val *InsertExpr) *InsertExpr {
	...
	in.hasher.HashFastPathUniqueChecksExpr(val.FastPathUniqueChecks)
	...
	for in.cache.Next() {
		if existing, ok := in.cache.Item().(*InsertExpr); ok {
			if in.hasher.IsRelExprEqual(val.Input, existing.Input) &&
				...
				in.hasher.IsFastPathUniqueChecksExprEqual(val.FastPathUniqueChecks, existing.FastPathUniqueChecks) &&
...
func (in *interner) InternFastPathUniqueChecks(val *FastPathUniqueChecksExpr) *FastPathUniqueChecksExpr {
	in.hasher.Init()
	in.hasher.HashOperator(opt.FastPathUniqueChecksOp)
	in.hasher.HashFastPathUniqueChecksExpr(*val)

	in.cache.Start(in.hasher.hash)
	for in.cache.Next() {
		if existing, ok := in.cache.Item().(*FastPathUniqueChecksExpr); ok {
			if in.hasher.IsFastPathUniqueChecksExprEqual(*val, *existing) {
				return existing
			}
		}
	}
...
func (in *interner) InternFastPathUniqueChecksItem(val *FastPathUniqueChecksItem) *FastPathUniqueChecksItem {
	in.hasher.Init()
	in.hasher.HashOperator(opt.FastPathUniqueChecksItemOp)
	in.hasher.HashRelExpr(val.Check)
	in.hasher.HashTableID(val.ReferencedTableID)
	in.hasher.HashIndexOrdinal(val.ReferencedIndexOrdinal)
	in.hasher.HashInt(val.CheckOrdinal)
	in.hasher.HashColList(val.InsertCols)
	in.hasher.HashScalarListExpr(val.DatumsFromConstraint)
	in.hasher.HashLocking(val.Locking)

	in.cache.Start(in.hasher.hash)
	for in.cache.Next() {
		if existing, ok := in.cache.Item().(*FastPathUniqueChecksItem); ok {
			if in.hasher.IsRelExprEqual(val.Check, existing.Check) &&
				in.hasher.IsTableIDEqual(val.ReferencedTableID, existing.ReferencedTableID) &&
				in.hasher.IsIndexOrdinalEqual(val.ReferencedIndexOrdinal, existing.ReferencedIndexOrdinal) &&
				in.hasher.IsIntEqual(val.CheckOrdinal, existing.CheckOrdinal) &&
				in.hasher.IsColListEqual(val.InsertCols, existing.InsertCols) &&
				in.hasher.IsScalarListExprEqual(val.DatumsFromConstraint, existing.DatumsFromConstraint) &&
...
func (in *interner) InternLock(val *LockExpr) *LockExpr {
	in.hasher.Init()
	in.hasher.HashOperator(opt.LockOp)
	in.hasher.HashRelExpr(val.Input)
	in.hasher.HashTableID(val.Table)
	in.hasher.HashTableID(val.KeySource)
	in.hasher.HashLocking(val.Locking)
	in.hasher.HashColList(val.KeyCols)
	in.hasher.HashColSet(val.LockCols)
	in.hasher.HashColSet(val.Cols)

	in.cache.Start(in.hasher.hash)
	for in.cache.Next() {
		if existing, ok := in.cache.Item().(*LockExpr); ok {
			if in.hasher.IsRelExprEqual(val.Input, existing.Input) &&
				in.hasher.IsTableIDEqual(val.Table, existing.Table) &&
				in.hasher.IsTableIDEqual(val.KeySource, existing.KeySource) &&
				in.hasher.IsLockingEqual(val.Locking, existing.Locking) &&
				in.hasher.IsColListEqual(val.KeyCols, existing.KeyCols) &&
				in.hasher.IsColSetEqual(val.LockCols, existing.LockCols) &&
...
func (in *interner) InternOrdinality(val *OrdinalityExpr) *OrdinalityExpr {
	...
	in.hasher.HashBool(val.ForDuplicateRemoval)
	...
	for in.cache.Next() {
		if existing, ok := in.cache.Item().(*OrdinalityExpr); ok {
			if in.hasher.IsRelExprEqual(val.Input, existing.Input) &&
				...
				in.hasher.IsBoolEqual(val.ForDuplicateRemoval, existing.ForDuplicateRemoval) {
				return existing
			}
		}
	}
...
func (in *interner) InternBarrier(val *BarrierExpr) *BarrierExpr {
	in.hasher.Init()
	in.hasher.HashOperator(opt.BarrierOp)
	in.hasher.HashRelExpr(val.Input)

	in.cache.Start(in.hasher.hash)
	for in.cache.Next() {
		if existing, ok := in.cache.Item().(*BarrierExpr); ok {
			if in.hasher.IsRelExprEqual(val.Input, existing.Input) {
				return existing
			}
		}
...
func (in *interner) InternSubquery(val *SubqueryExpr) *SubqueryExpr {
	...
	in.hasher.HashBool(val.WithinUDF)
	...
	for in.cache.Next() {
		if existing, ok := in.cache.Item().(*SubqueryExpr); ok {
			if in.hasher.IsRelExprEqual(val.Input, existing.Input) &&
				...
				in.hasher.IsBoolEqual(val.WithinUDF, existing.WithinUDF) &&
...
func (in *interner) InternExists(val *ExistsExpr) *ExistsExpr {
	...
	in.hasher.HashRelExpr(val.Input)
	in.hasher.HashColumnID(val.LazyEvalProjectionCol)
	in.hasher.HashPointer(unsafe.Pointer(val.OriginalExpr))
	in.hasher.HashOrdering(val.Ordering)
	in.hasher.HashColumnID(val.RequestedCol)
	in.hasher.HashBool(val.WithinUDF)
	...
	for in.cache.Next() {
		if existing, ok := in.cache.Item().(*ExistsExpr); ok {
			if in.hasher.IsRelExprEqual(val.Input, existing.Input) &&
				in.hasher.IsColumnIDEqual(val.LazyEvalProjectionCol, existing.LazyEvalProjectionCol) &&
				in.hasher.IsPointerEqual(unsafe.Pointer(val.OriginalExpr), unsafe.Pointer(existing.OriginalExpr)) &&
				in.hasher.IsOrderingEqual(val.Ordering, existing.Ordering) &&
				in.hasher.IsColumnIDEqual(val.RequestedCol, existing.RequestedCol) &&
				in.hasher.IsBoolEqual(val.WithinUDF, existing.WithinUDF) &&
...
func (in *interner) InternMergeAggregatedStmtMetadata(val *MergeAggregatedStmtMetadataExpr) *MergeAggregatedStmtMetadataExpr {
	in.hasher.Init()
	in.hasher.HashOperator(opt.MergeAggregatedStmtMetadataOp)
	in.hasher.HashScalarExpr(val.Input)

	in.cache.Start(in.hasher.hash)
	for in.cache.Next() {
		if existing, ok := in.cache.Item().(*MergeAggregatedStmtMetadataExpr); ok {
			if in.hasher.IsScalarExprEqual(val.Input, existing.Input) {
				return existing
			}
		}
...
func (in *interner) InternMergeStatsMetadata(val *MergeStatsMetadataExpr) *MergeStatsMetadataExpr {
	in.hasher.Init()
	in.hasher.HashOperator(opt.MergeStatsMetadataOp)
	in.hasher.HashScalarExpr(val.Input)

	in.cache.Start(in.hasher.hash)
	for in.cache.Next() {
		if existing, ok := in.cache.Item().(*MergeStatsMetadataExpr); ok {
			if in.hasher.IsScalarExprEqual(val.Input, existing.Input) {
				return existing
			}
		}
...
func (in *interner) InternMergeStatementStats(val *MergeStatementStatsExpr) *MergeStatementStatsExpr {
	in.hasher.Init()
	in.hasher.HashOperator(opt.MergeStatementStatsOp)
	in.hasher.HashScalarExpr(val.Input)

	in.cache.Start(in.hasher.hash)
	for in.cache.Next() {
		if existing, ok := in.cache.Item().(*MergeStatementStatsExpr); ok {
			if in.hasher.IsScalarExprEqual(val.Input, existing.Input) {
				return existing
			}
		}
...
func (in *interner) InternMergeTransactionStats(val *MergeTransactionStatsExpr) *MergeTransactionStatsExpr {
	in.hasher.Init()
	in.hasher.HashOperator(opt.MergeTransactionStatsOp)
	in.hasher.HashScalarExpr(val.Input)

	in.cache.Start(in.hasher.hash)
	for in.cache.Next() {
		if existing, ok := in.cache.Item().(*MergeTransactionStatsExpr); ok {
			if in.hasher.IsScalarExprEqual(val.Input, existing.Input) {
				return existing
			}
		}
...
func (in *interner) InternUDFCall(val *UDFCallExpr) *UDFCallExpr {
	in.hasher.Init()
	in.hasher.HashOperator(opt.UDFCallOp)
	in.hasher.HashScalarListExpr(val.Args)
	in.hasher.HashUDFDefinition(val.Def)

	in.cache.Start(in.hasher.hash)
	for in.cache.Next() {
		if existing, ok := in.cache.Item().(*UDFCallExpr); ok {
			if in.hasher.IsScalarListExprEqual(val.Args, existing.Args) &&
				in.hasher.IsUDFDefinitionEqual(val.Def, existing.Def) {
				return existing
			}
		}
	}
...
func (in *interner) InternTxnControl(val *TxnControlExpr) *TxnControlExpr {
	in.hasher.Init()
	in.hasher.HashOperator(opt.TxnControlOp)
	in.hasher.HashScalarListExpr(val.Args)
	in.hasher.HashStoredProcTxnOp(val.TxnOp)
	in.hasher.HashTransactionModes(val.TxnModes)
	...
	in.cache.Start(in.hasher.hash)
	for in.cache.Next() {
		if existing, ok := in.cache.Item().(*TxnControlExpr); ok {
			if in.hasher.IsScalarListExprEqual(val.Args, existing.Args) &&
				in.hasher.IsStoredProcTxnOpEqual(val.TxnOp, existing.TxnOp) &&
				in.hasher.IsTransactionModesEqual(val.TxnModes, existing.TxnModes) &&
...
func (in *interner) InternCreateFunction(val *CreateFunctionExpr) *CreateFunctionExpr {
	...
	in.hasher.HashSchemaDeps(val.Deps)
	in.hasher.HashSchemaTypeDeps(val.TypeDeps)
	in.hasher.HashSchemaFunctionDeps(val.FuncDeps)
	...
	for in.cache.Next() {
		if existing, ok := in.cache.Item().(*CreateFunctionExpr); ok {
			if in.hasher.IsSchemaIDEqual(val.Schema, existing.Schema) &&
				...
				in.hasher.IsSchemaDepsEqual(val.Deps, existing.Deps) &&
				in.hasher.IsSchemaTypeDepsEqual(val.TypeDeps, existing.TypeDeps) &&
				in.hasher.IsSchemaFunctionDepsEqual(val.FuncDeps, existing.FuncDeps) {
				return existing
			}
		}
	}
...
func (in *interner) InternCall(val *CallExpr) *CallExpr {
	in.hasher.Init()
	in.hasher.HashOperator(opt.CallOp)
	in.hasher.HashScalarExpr(val.Proc)
	in.hasher.HashColList(val.Columns)

	in.cache.Start(in.hasher.hash)
	for in.cache.Next() {
		if existing, ok := in.cache.Item().(*CallExpr); ok {
			if in.hasher.IsScalarExprEqual(val.Proc, existing.Proc) &&
				in.hasher.IsColListEqual(val.Columns, existing.Columns) {
				return existing
			}
		}
