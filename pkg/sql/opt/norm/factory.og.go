...
SKIP_RULES:
	e := _f.mem.MemoizeInsert(input, uniqueChecks, fastPathUniqueChecks, fKChecks, mutationPrivate)
	expr := _f.onConstructRelational(e)
	_f.constructorStackDepth--
	return expr
}
		...
		if _project != nil {
			...
			if !_f.funcs.ColsAreEmpty(simplifiableCols) {
				if _f.matchedRule == nil || _f.matchedRule(opt.SimplifyPartialIndexProjections) {
					simplifiedProjections, simplifiedPrivate := _f.funcs.SimplifyPartialIndexProjections(projections, passthrough, simplifiableCols, mutationPrivate)
...
// ConstructFastPathUniqueChecksItem constructs an expression for the FastPathUniqueChecksItem operator.
...
func (_f *Factory) ConstructFastPathUniqueChecksItem(
	check memo.RelExpr,
	fastPathUniqueChecksItemPrivate *memo.FastPathUniqueChecksItemPrivate,
) memo.FastPathUniqueChecksItem {
	opt.MaybeInjectOptimizerTestingPanic(_f.ctx, _f.evalCtx)
	item := memo.FastPathUniqueChecksItem{Check: check, FastPathUniqueChecksItemPrivate: *fastPathUniqueChecksItemPrivate}
	return item
}
...
// ConstructLock constructs an expression for the Lock operator.
...
func (_f *Factory) ConstructLock(
	input memo.RelExpr,
	lockPrivate *memo.LockPrivate,
) memo.RelExpr {
	opt.MaybeInjectOptimizerTestingPanic(_f.ctx, _f.evalCtx)
	_f.constructorStackDepth++
	if _f.constructorStackDepth > maxConstructorStackDepth {
...
SKIP_RULES:
	e := _f.mem.MemoizeLock(input, lockPrivate)
	expr := _f.onConstructRelational(e)
	_f.constructorStackDepth--
	return expr
}
...
func (_f *Factory) ConstructSelect(
	...
	// [TryRemapSelectOuterCols]
	{
		if _f.funcs.HasOuterCols(input) {
			on := filters
			if _f.funcs.CanMaybeRemapOuterCols(input, on) {
				remapped, ok := _f.funcs.TryRemapOuterCols(input, on)
				if ok {
					if _f.matchedRule == nil || _f.matchedRule(opt.TryRemapSelectOuterCols) {
						_expr := _f.ConstructSelect(
							remapped,
							on,
						)
						if _f.appliedRule != nil {
							_f.appliedRule(opt.TryRemapSelectOuterCols, nil, _expr)
						}
						_f.constructorStackDepth--
						return _expr
					}
				...
				if _f.matchedRule == nil || _f.matchedRule(opt.NormalizeSelectAnyFilter) {
					_expr := _f.ConstructSelect(
						...
						_f.funcs.ReplaceFiltersItem(filters, item, _f.ConstructExists(
							...
							_f.funcs.ConvertSubToExistsPrivate(anyPrivate),
						)),
					)
					if _f.appliedRule != nil {
	...
	// [PushSelectIntoOrdinality]
	{
		...
		if _ordinality != nil {
			...
			if _f.funcs.ForDuplicateRemoval(private) {
				for i := range filters {
					item := &filters[i]
					inputCols := _f.funcs.OutputCols(input)
					if _f.funcs.IsBoundBy(item, inputCols) {
						if _f.matchedRule == nil || _f.matchedRule(opt.PushSelectIntoOrdinality) {
							_expr := _f.ConstructSelect(
							...
							if _f.appliedRule != nil {
								_f.appliedRule(opt.PushSelectIntoOrdinality, nil, _expr)
							}
							_f.constructorStackDepth--
							return _expr
						}
		...
		if input.Op() == opt.LeftJoinOp || input.Op() == opt.LeftJoinApplyOp || input.Op() == opt.SemiJoinOp || input.Op() == opt.SemiJoinApplyOp || input.Op() == opt.AntiJoinOp || input.Op() == opt.AntiJoinApplyOp {
			...
			for i := range filters {
				...
				if _f.funcs.IsBoundBy(item, _f.funcs.OutputCols(left)) {
					...
					equivSet := _f.funcs.GetEquivSet(on, left, right)
					if _f.funcs.CanMapJoinOpFilter(item, rightCols, equivSet) {
							...
							_expr := _f.ConstructSelect(
								_f.DynamicConstruct(
									...
									_f.ConstructSelect(
										...
										memo.FiltersExpr{
											_f.ConstructFiltersItem(
												_f.funcs.MapJoinOpFilter(item, rightCols, equivSet),
											),
										},
									),
									&on,
...
func (_f *Factory) ConstructProject(
		...
		if join.Op() == opt.InnerJoinOp || join.Op() == opt.LeftJoinOp {
			...
			if _f.funcs.JoinDoesNotDuplicateLeftRows(join) {
				if _f.funcs.JoinPreservesLeftRows(join) {
					fromCols := _f.funcs.UnionCols(passthrough, _f.funcs.ProjectionOuterCols(projections))
					leftCols := _f.funcs.OutputCols(left)
					fds := _f.funcs.FuncDeps(join)
					if _f.funcs.CanRemapCols(fromCols, leftCols, fds) {
						if _f.funcs.CanUseImprovedJoinElimination(fromCols, leftCols) {
							if _f.matchedRule == nil || _f.matchedRule(opt.EliminateJoinUnderProjectLeft) {
								_expr := _f.ConstructProject(
									left,
									_f.funcs.MergeProjections(_f.funcs.RemapProjectionCols(projections, leftCols, fds), _f.funcs.ProjectRemappedCols(passthrough, leftCols, fds), passthrough),
									_f.funcs.DifferenceCols(passthrough, _f.funcs.OutputCols(right)),
								)
								if _f.appliedRule != nil {
		...
		if _innerJoin != nil {
			...
			if _f.funcs.JoinDoesNotDuplicateRightRows(_innerJoin) {
				if _f.funcs.JoinPreservesRightRows(_innerJoin) {
					fromCols := _f.funcs.UnionCols(passthrough, _f.funcs.ProjectionOuterCols(projections))
					rightCols := _f.funcs.OutputCols(right)
					fds := _f.funcs.FuncDeps(_innerJoin)
					if _f.funcs.CanRemapCols(fromCols, rightCols, fds) {
						if _f.funcs.CanUseImprovedJoinElimination(fromCols, rightCols) {
							if _f.matchedRule == nil || _f.matchedRule(opt.EliminateJoinUnderProjectRight) {
								_expr := _f.ConstructProject(
									right,
									_f.funcs.MergeProjections(_f.funcs.RemapProjectionCols(projections, rightCols, fds), _f.funcs.ProjectRemappedCols(passthrough, rightCols, fds), passthrough),
									_f.funcs.DifferenceCols(passthrough, _f.funcs.OutputCols(left)),
								)
								if _f.appliedRule != nil {
	...
	{
		if input.Op() == opt.UpdateOp || input.Op() == opt.UpsertOp || input.Op() == opt.DeleteOp {
			innerInput := input.Child(0).(memo.RelExpr)
	...
	{
		_insert, _ := input.(*memo.InsertExpr)
		if _insert != nil {
			innerInput := _insert.Input
			uniqueChecks := _insert.UniqueChecks
			fastPathUniqueChecks := _insert.FastPathUniqueChecks
			fkChecks := _insert.FKChecks
			mutationPrivate := &_insert.MutationPrivate
			needed := _f.funcs.UnionCols3(_f.funcs.PrimaryKeyCols(_f.funcs.MutationTable(mutationPrivate)), _f.funcs.ProjectionOuterCols(projections), passthrough)
			if _f.funcs.CanPruneMutationReturnCols(mutationPrivate, needed) {
				if _f.matchedRule == nil || _f.matchedRule(opt.PruneInsertReturnCols) {
					_expr := _f.ConstructProject(
						_f.ConstructInsert(
							innerInput,
							uniqueChecks,
					...
					if _f.appliedRule != nil {
						_f.appliedRule(opt.PruneInsertReturnCols, nil, _expr)
					}
					_f.constructorStackDepth--
					return _expr
				}
...
func (_f *Factory) ConstructInnerJoin(
	...
	// [TryRemapJoinOuterColsRight]
	{
		if _f.funcs.HasOuterCols(right) {
			if _f.funcs.CanMaybeRemapOuterCols(right, on) {
				remapped, ok := _f.funcs.TryRemapOuterCols(right, on)
				if ok {
					private := joinPrivate
					if _f.matchedRule == nil || _f.matchedRule(opt.TryRemapJoinOuterColsRight) {
						_expr := _f.ConstructInnerJoin(
							left,
							remapped,
							on,
						...
						if _f.appliedRule != nil {
							_f.appliedRule(opt.TryRemapJoinOuterColsRight, nil, _expr)
						}
						_f.constructorStackDepth--
						return _expr
					}
	...
	// [TryRemapJoinOuterColsLeft]
	{
		if _f.funcs.HasOuterCols(left) {
			if _f.funcs.CanMaybeRemapOuterCols(left, on) {
				remapped, ok := _f.funcs.TryRemapOuterCols(left, on)
				if ok {
					private := joinPrivate
					if _f.matchedRule == nil || _f.matchedRule(opt.TryRemapJoinOuterColsLeft) {
						_expr := _f.ConstructInnerJoin(
							remapped,
							right,
							on,
						...
						if _f.appliedRule != nil {
							_f.appliedRule(opt.TryRemapJoinOuterColsLeft, nil, _expr)
						}
						_f.constructorStackDepth--
						return _expr
					}
			...
			if !_f.funcs.HasOuterCols(right) {
				for i := range on {
					...
					if !_match {
						leftCols := _f.funcs.OutputCols(left)
						equivSet := _f.funcs.GetEquivSet(on, left, right)
						if _f.funcs.CanMapJoinOpFilter(item, leftCols, equivSet) {
							rightCols := _f.funcs.OutputCols(right)
							if _f.funcs.CanMapJoinOpFilter(item, rightCols, equivSet) {
								private := joinPrivate
								if _f.matchedRule == nil || _f.matchedRule(opt.PushFilterIntoJoinLeftAndRight) {
									_expr := _f.ConstructInnerJoin(
										_f.ConstructSelect(
											...
											memo.FiltersExpr{
												_f.ConstructFiltersItem(
													_f.funcs.MapJoinOpFilter(item, leftCols, equivSet),
												),
											},
										),
										_f.ConstructSelect(
						...
						if _f.funcs.CanMapJoinOpFilter(item, leftCols, equivSet) {
							...
							if _f.matchedRule == nil || _f.matchedRule(opt.MapFilterIntoJoinLeft) {
								_expr := _f.ConstructInnerJoin(
									left,
									right,
									_f.funcs.ReplaceFiltersItem(on, item, _f.funcs.MapJoinOpFilter(item, leftCols, equivSet)),
									private,
								)
								if _f.appliedRule != nil {
						...
						if _f.funcs.CanMapJoinOpFilter(item, rightCols, equivSet) {
							...
							if _f.matchedRule == nil || _f.matchedRule(opt.MapFilterIntoJoinRight) {
								_expr := _f.ConstructInnerJoin(
									left,
									right,
									_f.funcs.ReplaceFiltersItem(on, item, _f.funcs.MapJoinOpFilter(item, rightCols, equivSet)),
									private,
								)
								if _f.appliedRule != nil {
...
func (_f *Factory) ConstructGroupBy(
	...
	// [EliminateJoinUnderGroupByLeft]
	{
		if input.Op() == opt.InnerJoinOp || input.Op() == opt.LeftJoinOp {
			...
			if _f.funcs.OrderingCanProjectCols(ordering, leftCols) {
				toRemap := _f.funcs.UnionCols(groupingCols, _f.funcs.AggregationOuterCols(aggs))
				fds := _f.funcs.FuncDeps(input)
				if _f.funcs.CanRemapCols(toRemap, leftCols, fds) {
					if _f.funcs.CanUseImprovedJoinElimination(toRemap, leftCols) {
						if _f.funcs.CanEliminateJoinUnderGroupByLeft(input, aggs) {
							if _f.matchedRule == nil || _f.matchedRule(opt.EliminateJoinUnderGroupByLeft) {
								_expr := _f.ConstructGroupBy(
									_f.ConstructProject(
										left,
										_f.funcs.ProjectRemappedCols(toRemap, leftCols, fds),
										leftCols,
									),
									aggs,
	...
	// [EliminateJoinUnderGroupByRight]
	{
		...
		if _innerJoin != nil {
			...
			if _f.funcs.OrderingCanProjectCols(ordering, rightCols) {
				toRemap := _f.funcs.UnionCols(groupingCols, _f.funcs.AggregationOuterCols(aggs))
				fds := _f.funcs.FuncDeps(_innerJoin)
				if _f.funcs.CanRemapCols(toRemap, rightCols, fds) {
					if _f.funcs.CanUseImprovedJoinElimination(toRemap, rightCols) {
						if _f.funcs.CanEliminateJoinUnderGroupByRight(_innerJoin, aggs) {
							if _f.matchedRule == nil || _f.matchedRule(opt.EliminateJoinUnderGroupByRight) {
								_expr := _f.ConstructGroupBy(
									_f.ConstructProject(
										right,
										_f.funcs.ProjectRemappedCols(toRemap, rightCols, fds),
										rightCols,
									),
									aggs,
	...
	{
		aggs := aggregations
		if _f.funcs.AreAllAnyNotNullAggs(aggs) {
			if _f.funcs.ColsAreStrictKey(_f.funcs.GroupingCols(groupingPrivate), input) {
				if _f.matchedRule == nil || _f.matchedRule(opt.EliminateGroupBy) {
					_expr := _f.ConstructProject(
						input,
						_f.funcs.ConvertAnyNotNullAggsToProjections(aggs),
						_f.funcs.IntersectionCols(_f.funcs.GroupingOutputCols(groupingPrivate, aggs), _f.funcs.OutputCols(input)),
					)
					if _f.appliedRule != nil {
						_f.appliedRule(opt.EliminateGroupBy, nil, _expr)
					}
					_f.constructorStackDepth--
					return _expr
				}
...
func (_f *Factory) ConstructUpsertDistinctOn(
	...
	{
		aggs := aggregations
		if _f.funcs.ColsAreLaxKey(_f.funcs.GroupingCols(groupingPrivate), input) {
			if _f.matchedRule == nil || _f.matchedRule(opt.EliminateUpsertDistinct) {
				_expr := _f.ConstructProject(
					input,
					memo.EmptyProjectionsExpr,
					_f.funcs.GroupingOutputCols(groupingPrivate, aggs),
				...
				if _f.appliedRule != nil {
					_f.appliedRule(opt.EliminateUpsertDistinct, nil, _expr)
				}
				_f.constructorStackDepth--
				return _expr
			}
...
// ConstructBarrier constructs an expression for the Barrier operator.
...
func (_f *Factory) ConstructBarrier(
	input memo.RelExpr,
) memo.RelExpr {
	opt.MaybeInjectOptimizerTestingPanic(_f.ctx, _f.evalCtx)
	_f.constructorStackDepth++
			...
			if len(_values.Rows) == 1 {
				...
				if _tuple != nil {
					if len(_tuple.Elems) == 1 {
						value := _tuple.Elems[0]
						_const, _ := value.(*memo.ConstExpr)
						if _const != nil {
							if _f.matchedRule == nil || _f.matchedRule(opt.EliminateConstValueSubquery) {
								_expr := _const
								if _f.appliedRule != nil {
									_f.appliedRule(opt.EliminateConstValueSubquery, nil, _expr)
								}
								_f.constructorStackDepth--
								return _expr
							}
...
func (_f *Factory) ConstructExists(
	input memo.RelExpr,
	existsPrivate *memo.ExistsPrivate,
) opt.ScalarExpr {
	opt.MaybeInjectOptimizerTestingPanic(_f.ctx, _f.evalCtx)
	_f.constructorStackDepth++
	if _f.constructorStackDepth > maxConstructorStackDepth {
	...
	{
		if !_f.funcs.HasOuterCols(input) {
			if _f.matchedRule == nil || _f.matchedRule(opt.ConvertUncorrelatedExistsToCoalesceSubquery) {
						...
						_f.ConstructSubquery(
							_f.ConstructProject(
								...
								memo.ProjectionsExpr{
									_f.ConstructProjectionsItem(
										_f.ConstructTrue(),
										_f.funcs.MakeBoolCol(),
									),
								},
								_f.funcs.MakeEmptyColSet(),
							),
							_f.funcs.EmbeddedSubqueryPrivate(existsPrivate),
						),
						_f.ConstructFalse(),
					},
				)
				if _f.appliedRule != nil {
					_f.appliedRule(opt.ConvertUncorrelatedExistsToCoalesceSubquery, nil, _expr)
				}
				_f.constructorStackDepth--
				return _expr
			}
...
SKIP_RULES:
	e := _f.mem.MemoizeExists(input, existsPrivate)
	expr := _f.onConstructScalar(e)
	_f.constructorStackDepth--
	return expr
}
...
func (_f *Factory) ConstructArrayFlatten(
	...
	{
		private := subqueryPrivate
		if _f.funcs.CanNormalizeArrayFlatten(input, private) {
			if _f.matchedRule == nil || _f.matchedRule(opt.NormalizeArrayFlattenToAgg) {
				requestedCol := _f.funcs.SubqueryRequestedCol(private)
				_expr := _f.ConstructCoalesce(
					memo.ScalarListExpr{
						_f.ConstructSubquery(
							_f.ConstructScalarGroupBy(
								...
								_f.funcs.MakeGrouping(_f.funcs.MakeEmptyColSet(), _f.funcs.SubqueryOrdering(private)),
							),
...
// ConstructMergeAggregatedStmtMetadata constructs an expression for the MergeAggregatedStmtMetadata operator.
func (_f *Factory) ConstructMergeAggregatedStmtMetadata(
	input opt.ScalarExpr,
) opt.ScalarExpr {
	opt.MaybeInjectOptimizerTestingPanic(_f.ctx, _f.evalCtx)
	_f.constructorStackDepth++
...
SKIP_RULES:
	e := _f.mem.MemoizeMergeAggregatedStmtMetadata(input)
	expr := _f.onConstructScalar(e)
	_f.constructorStackDepth--
	return expr
}

// ConstructMergeStatsMetadata constructs an expression for the MergeStatsMetadata operator.
func (_f *Factory) ConstructMergeStatsMetadata(
	input opt.ScalarExpr,
) opt.ScalarExpr {
	opt.MaybeInjectOptimizerTestingPanic(_f.ctx, _f.evalCtx)
	_f.constructorStackDepth++
...
SKIP_RULES:
	e := _f.mem.MemoizeMergeStatsMetadata(input)
	expr := _f.onConstructScalar(e)
	_f.constructorStackDepth--
	return expr
}

// ConstructMergeStatementStats constructs an expression for the MergeStatementStats operator.
func (_f *Factory) ConstructMergeStatementStats(
	input opt.ScalarExpr,
) opt.ScalarExpr {
	opt.MaybeInjectOptimizerTestingPanic(_f.ctx, _f.evalCtx)
	_f.constructorStackDepth++
...
SKIP_RULES:
	e := _f.mem.MemoizeMergeStatementStats(input)
	expr := _f.onConstructScalar(e)
	_f.constructorStackDepth--
	return expr
}

// ConstructMergeTransactionStats constructs an expression for the MergeTransactionStats operator.
func (_f *Factory) ConstructMergeTransactionStats(
	input opt.ScalarExpr,
) opt.ScalarExpr {
	opt.MaybeInjectOptimizerTestingPanic(_f.ctx, _f.evalCtx)
	_f.constructorStackDepth++
...
SKIP_RULES:
	e := _f.mem.MemoizeMergeTransactionStats(input)
	expr := _f.onConstructScalar(e)
	_f.constructorStackDepth--
	return expr
}
...
// ConstructUDFCall constructs an expression for the UDFCall operator.
// UDFCall invokes a user-defined function. The UDFPrivate field contains a
// pointer to the definition of the UDF.
func (_f *Factory) ConstructUDFCall(
	args memo.ScalarListExpr,
	uDFCallPrivate *memo.UDFCallPrivate,
) opt.ScalarExpr {
	opt.MaybeInjectOptimizerTestingPanic(_f.ctx, _f.evalCtx)
	_f.constructorStackDepth++
	if _f.constructorStackDepth > maxConstructorStackDepth {
...
SKIP_RULES:
	e := _f.mem.MemoizeUDFCall(args, uDFCallPrivate)
	expr := _f.onConstructScalar(e)
	_f.constructorStackDepth--
	return expr
}

// ConstructTxnControl constructs an expression for the TxnControl operator.
...
// transaction.
func (_f *Factory) ConstructTxnControl(
	args memo.ScalarListExpr,
	txnControlPrivate *memo.TxnControlPrivate,
) opt.ScalarExpr {
	opt.MaybeInjectOptimizerTestingPanic(_f.ctx, _f.evalCtx)
	_f.constructorStackDepth++
	if _f.constructorStackDepth > maxConstructorStackDepth {
...
SKIP_RULES:
	e := _f.mem.MemoizeTxnControl(args, txnControlPrivate)
	expr := _f.onConstructScalar(e)
	_f.constructorStackDepth--
	return expr
}
...
// ConstructCall constructs an expression for the Call operator.
func (_f *Factory) ConstructCall(
	proc opt.ScalarExpr,
	callPrivate *memo.CallPrivate,
) memo.RelExpr {
	opt.MaybeInjectOptimizerTestingPanic(_f.ctx, _f.evalCtx)
	_f.constructorStackDepth++
	if _f.constructorStackDepth > maxConstructorStackDepth {
...
SKIP_RULES:
	e := _f.mem.MemoizeCall(proc, callPrivate)
	expr := _f.onConstructRelational(e)
	_f.constructorStackDepth--
	return expr
}
...
func (f *Factory) Replace(e opt.Expr, replace ReplaceFunc) opt.Expr {
	...
	case *memo.InsertExpr:
		...
		fastPathUniqueChecks, fastPathUniqueChecksChanged := f.replaceFastPathUniqueChecksExpr(t.FastPathUniqueChecks, replace)
		fKChecks, fKChecksChanged := f.replaceFKChecksExpr(t.FKChecks, replace)
		if input != t.Input || uniqueChecksChanged || fastPathUniqueChecksChanged || fKChecksChanged {
			return f.ConstructInsert(input, uniqueChecks, fastPathUniqueChecks, fKChecks, &t.MutationPrivate)
		}
		return t
	...
	case *memo.FastPathUniqueChecksExpr:
		if after, changed := f.replaceFastPathUniqueChecksExpr(*t, replace); changed {
			return &after
		}
		return t
	...
	case *memo.LockExpr:
		input := replace(t.Input).(memo.RelExpr)
		if input != t.Input {
			return f.ConstructLock(input, &t.LockPrivate)
		}
		return t
	...
	case *memo.BarrierExpr:
		input := replace(t.Input).(memo.RelExpr)
		if input != t.Input {
			return f.ConstructBarrier(input)
		}
		return t
	...
	case *memo.ExistsExpr:
		input := replace(t.Input).(memo.RelExpr)
		if input != t.Input {
			return f.ConstructExists(input, &t.ExistsPrivate)
		}
		return t
	...
	case *memo.MergeAggregatedStmtMetadataExpr:
		input := replace(t.Input).(opt.ScalarExpr)
		if input != t.Input {
			return f.ConstructMergeAggregatedStmtMetadata(input)
		}
		return t

	case *memo.MergeStatsMetadataExpr:
		input := replace(t.Input).(opt.ScalarExpr)
		if input != t.Input {
			return f.ConstructMergeStatsMetadata(input)
		}
		return t

	case *memo.MergeStatementStatsExpr:
		input := replace(t.Input).(opt.ScalarExpr)
		if input != t.Input {
			return f.ConstructMergeStatementStats(input)
		}
		return t

	case *memo.MergeTransactionStatsExpr:
		input := replace(t.Input).(opt.ScalarExpr)
		if input != t.Input {
			return f.ConstructMergeTransactionStats(input)
		}
		return t
	...
	case *memo.UDFCallExpr:
		args, argsChanged := f.replaceScalarListExpr(t.Args, replace)
		if argsChanged {
			return f.ConstructUDFCall(args, &t.UDFCallPrivate)
		}
		return t

	case *memo.TxnControlExpr:
		args, argsChanged := f.replaceScalarListExpr(t.Args, replace)
		if argsChanged {
			return f.ConstructTxnControl(args, &t.TxnControlPrivate)
		}
		return t
	...
	case *memo.CallExpr:
		proc := replace(t.Proc).(opt.ScalarExpr)
		if proc != t.Proc {
			return f.ConstructCall(proc, &t.CallPrivate)
		}
		return t
...
func (f *Factory) replaceFastPathUniqueChecksExpr(list memo.FastPathUniqueChecksExpr, replace ReplaceFunc) (_ memo.FastPathUniqueChecksExpr, changed bool) {
	var newList []memo.FastPathUniqueChecksItem
	for i := range list {
		before := list[i].Check
		after := replace(before).(memo.RelExpr)
		if before != after {
			if newList == nil {
				newList = make([]memo.FastPathUniqueChecksItem, len(list))
				copy(newList, list[:i])
			}
			newList[i] = f.ConstructFastPathUniqueChecksItem(after, &list[i].FastPathUniqueChecksItemPrivate)
		} else if newList != nil {
			newList[i] = list[i]
		}
	}
...
func (f *Factory) CopyAndReplaceDefault(src opt.Expr, replace ReplaceFunc) (dst opt.Expr) {
	...
	case *memo.InsertExpr:
		...
		return f.ConstructInsert(
			...
			f.copyAndReplaceDefaultFastPathUniqueChecksExpr(t.FastPathUniqueChecks, replace),
	...
	case *memo.MergeAggregatedStmtMetadataExpr:
		return f.ConstructMergeAggregatedStmtMetadata(
			f.invokeReplace(t.Input, replace).(opt.ScalarExpr),
		)

	case *memo.MergeStatsMetadataExpr:
		return f.ConstructMergeStatsMetadata(
			f.invokeReplace(t.Input, replace).(opt.ScalarExpr),
		)

	case *memo.MergeStatementStatsExpr:
		return f.ConstructMergeStatementStats(
			f.invokeReplace(t.Input, replace).(opt.ScalarExpr),
		)

	case *memo.MergeTransactionStatsExpr:
		return f.ConstructMergeTransactionStats(
			f.invokeReplace(t.Input, replace).(opt.ScalarExpr),
		)
	...
	case *memo.UDFCallExpr:
		return f.ConstructUDFCall(
			f.copyAndReplaceDefaultScalarListExpr(t.Args, replace),
			&t.UDFCallPrivate,
		)

	case *memo.TxnControlExpr:
		return f.ConstructTxnControl(
			f.copyAndReplaceDefaultScalarListExpr(t.Args, replace),
			&t.TxnControlPrivate,
		)
	...
	case *memo.CallExpr:
		return f.ConstructCall(
			f.invokeReplace(t.Proc, replace).(opt.ScalarExpr),
			&t.CallPrivate,
		)
	...
	case *memo.FastPathUniqueChecksExpr:
		newVal := f.copyAndReplaceDefaultFastPathUniqueChecksExpr(*t, replace)
		return &newVal
...
func (f *Factory) copyAndReplaceDefaultFastPathUniqueChecksExpr(src memo.FastPathUniqueChecksExpr, replace ReplaceFunc) (dst memo.FastPathUniqueChecksExpr) {
	dst = make(memo.FastPathUniqueChecksExpr, len(src))
	for i := range src {
		dst[i].Check = f.invokeReplace(src[i].Check, replace).(memo.RelExpr)
		dst[i].ReferencedTableID = src[i].ReferencedTableID
		dst[i].ReferencedIndexOrdinal = src[i].ReferencedIndexOrdinal
		dst[i].CheckOrdinal = src[i].CheckOrdinal
		dst[i].InsertCols = src[i].InsertCols
		dst[i].DatumsFromConstraint = src[i].DatumsFromConstraint
		dst[i].Locking = src[i].Locking
		f.mem.CheckExpr(&dst[i])
	}
	return dst
}
...
func (f *Factory) DynamicConstruct(op opt.Operator, args ...interface{}) opt.Expr {
	...
	case opt.InsertOp:
		return f.ConstructInsert(
			...
			*args[2].(*memo.FastPathUniqueChecksExpr),
			*args[3].(*memo.FKChecksExpr),
			args[4].(*memo.MutationPrivate),
		)
	case opt.UpdateOp:
		return f.ConstructUpdate(
			args[0].(memo.RelExpr),
	...
	case opt.MergeAggregatedStmtMetadataOp:
		return f.ConstructMergeAggregatedStmtMetadata(
			args[0].(opt.ScalarExpr),
		)
	case opt.MergeStatsMetadataOp:
		return f.ConstructMergeStatsMetadata(
			args[0].(opt.ScalarExpr),
		)
	case opt.MergeStatementStatsOp:
		return f.ConstructMergeStatementStats(
			args[0].(opt.ScalarExpr),
		)
	case opt.MergeTransactionStatsOp:
		return f.ConstructMergeTransactionStats(
			args[0].(opt.ScalarExpr),
		)
	...
	case opt.UDFCallOp:
		return f.ConstructUDFCall(
			*args[0].(*memo.ScalarListExpr),
			args[1].(*memo.UDFCallPrivate),
		)
	case opt.TxnControlOp:
		return f.ConstructTxnControl(
			*args[0].(*memo.ScalarListExpr),
			args[1].(*memo.TxnControlPrivate),
		)
